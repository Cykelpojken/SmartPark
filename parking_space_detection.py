from car_controller.car_controller import CarController  # pragma: no cover
import time
import numpy as np
import cv2
...
EROSION = 1  # Iteration. More means thicker
DILATION = 1  # Iteration. More means more is removed
MIN_MATCH_COUNT = 6
BOXBLUR = cfg.BOX_BLUR
SURF = True
SAVE_PICTURES = False
...
def blur(img, threshold, t=cv2.BORDER_DEFAULT):
    blur = cv2.GaussianBlur(img, (threshold, threshold), t)
    if SAVE_PICTURES:
        cv2.imwrite("blurred.jpg", blur)
    return blur

def find_spot(img=None, box=None):  # Working well ish
    parking_coordinates = None
    if box is None:
            box = cv2.imread('box4.png', 0)  # queryImage
            box = blur(box, BOXBLUR ,t = 0)  
    ...
    return img3, parking_coordinates, len(good)
