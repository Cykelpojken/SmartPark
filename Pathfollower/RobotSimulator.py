...
def draw_path(img):
    ...
    cv2.circle(img, (int(path[0][0]*scaler), int(path[0][1]*scaler)), 2,
               (255*(1-path[0][2]/float(config["VELOCITY"]["MAX_VEL"])), 0, 255*path[0][2] /
               float(config["VELOCITY"]["MAX_VEL"])), -1)
    for i in range(1, len(path)):
        cv2.circle(img, (int(path[i][0]*scaler), int(path[i][1]*scaler)), 2,
                   (255*(1-path[i-1][2]/float(config["VELOCITY"]["MAX_VEL"])), 0, 255*path[i-1][2] /
                   float(config["VELOCITY"]["MAX_VEL"])), -1)
...
def draw_robot(img):
    ...
    cv2.circle(tmp, (int(path[close][0]*scaler), int(path[close][1]*scaler)), 4,
               (255*(1-path[close][2]/float(config["VELOCITY"]["MAX_VEL"])), 0, 255*path[close][2] /
               float(config["VELOCITY"]["MAX_VEL"])), -1)
    ...
    try:
        ...
        cv2.circle(tmp, (int(x*scaler), int(y*scaler)), int(abs(1/curv*scaler)), (0, 255, 255), 1)  # No draw right pos.
